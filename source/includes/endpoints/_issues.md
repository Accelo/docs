...
| contract_id | The id of the [contract](#contracts) the issue is under, if this field is empty, the `contract_id` will be unset. This must point to a related contract, you can use the `is_realted_to_issue` filter on [`GET /contracts`](#list-contracts) to find related contracts. |
